...
type PiriPlugin (system: ExtendedActorSystem) =
    ...
    let vmmanager =
        if not (Directory.Exists config.AssemblyCachePath) 
        then Directory.CreateDirectory (config.AssemblyCachePath) |> ignore
        Vagabond.Initialize(
    ...
    /// Spawns an actor with behavior of tabaso host under curent /user root guardian
    member x.Server = spawn system "piri-server" (Server.receive vmmanager)
    /// Spawns an actor with behavior of tabaso client under curent /user root guardian
